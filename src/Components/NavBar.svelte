...
<div
    class="flex flex-col items-stretch justify-between dark:bg-secondary-d bg-secondary min-w-40 px-1 py-2 rounded-r-lg drop-shadow-xl"
>
    <!--Top Menu Items-->
    <div class="menu-container gap-0.5">
        {#each navbarContents.top as item}
            <button
                class="nav-btn min-w-30 flex flex-row text-center items-center justify-stretch gap-2"
    ...
    <div class="menu-container gap-0.5">
        {#each navbarContents.bottom as item}
            <button
                class="nav-btn min-w-30 flex flex-row text-center items-center justify-start gap-2"
                onmouseleave={(e) => e.currentTarget.blur()}
                onclick={async () => await navigateToPage(item.route)}
            >
                <item.icon class="w-5 h-5" />{item.name}
