...
export async function getMic() {
  if (localStorage.getItem("mic_granted") !== "true") {
    try {
      await message(
        "In order to function properly, you will need to allow this app to access your microphone",
      );
...
export async function attemptRelaunch() {
  ...
  if (isDev) {
    await message(
      "You are running a developer build of this app. This app will not restart.",
      {
...
/**
 * @description Checks if a FFMpeg filter list is valid
 * @param filterList FFMpeg filter list to verify
 */
