...
type ValidData =
  ...
  | "settings.recordStartDelay"
  | "settings.noiseSuppressionIntensity"
  | "settings.recordEndDelay"
  | "settings.enableTranscription"
  | "settings.enableRichPresence"
  | "settings.allowNoiseSuppression"
  | "settings.allowStatistics"
  | "settings.sortingMethod"
  | "statistics.filesRecorded"
  | "statistics.filesAccepted"
  | "statistics.filesRejected"
  | "statistics.filesSkipped"
  | "statistics.recordingsCancelled";
...
export async function setValue(
  ...
  const split = name.split(".");
  if (
    !(await getValue("settings.allowStatistics")) &&
    split[0] === "statistics"
  )
    return; // Checking here rather than in all places where the statistics stuff is implemented saves my time and produces less boilerplate
