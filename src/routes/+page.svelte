<script lang="ts">
    ...
    onMount(async () => {
        ...
        for (let i = 0; i < recentProjectPaths.length; i++) {
            recentProjectObjs.push(
                JSON.parse(await readTextFile(recentProjectPaths[i])),
            );
        }
    });
...
<div class="flex flex-col gap-3 h-full w-full p-3">
    <div class="flex flex-row gap-5 h-full">
        <div class="flex flex-col w-1/2 h-full card rounded-xl p-3">
            ...
            <div
                class="flex flex-col text-center justify-between items-center mt-15 h-full gap-y-3"
            >
                ...
                {:else}
                    <div class="w-full overflow-y-auto">
                        {#each recentProjectObjs as rp, i}
                            <button
                                class="save-btn rounded-sm w-full mb-1 dark:hover:bg-tertiary-d hover:bg-tertiary dark:focus:bg-tertiary-d dark:focus:drop-shadow-xl transition duration-75"
                                onclick={async () =>
                                    await loadProject(recentProjectPaths[i])}
                                onmouseleave={(e) => e.currentTarget.blur()}
                                onmouseup={(e) => e.currentTarget.blur()}
                            >
                                <div
                                    class="flex justify-between items-center text-left p-3 mb-1.5 mt-1.5"
                                >
                                    <div class="flex flex-col">
                                        <p class="text-lg">{rp.name}</p>
                                        <p class="text-gray-400 text-sm">
                                            Last Opened: {timestampToLegible(
                                                rp.lastOpened,
                                            )}
                                        </p>
                ...
                {/if}
                <div
                    class="flex row w-max mb-1 h-auto p-2 rounded-lgjustify-end align-bottom items-end gap-x-5"
                >
                    <button
                        class="text-center p-1.5 flex flex-row items-center justify-center gap-2 hover:bg-accent focus:bg-accent-tertiary rounded-md transition border-white/10 border"
                        onclick={newProject}
                        onmouseleave={(e) => e.currentTarget.blur()}
                        onmouseup={(e) => e.currentTarget.blur()}
                    >
        ...
        <div class="flex flex-col gap-y-5 w-1/2">
            <div
                class="card h-1/2 rounded-xl p-3 flex flex-col justify-center items-center"
            >
                <h1 class="text-center text-3xl font-medium mb-5">
                    New To Audio Replacer?
                </h1>
                <button
                    class="app-btn"
                    onmouseleave={(e) => e.currentTarget.blur()}
                    ...
                    onclick={async () => {
                        await openUrl(
                            "https://github.com/lemons-studios/audio-replacer/wiki",
                        );
                    }}
                    ><BookOpenText class="button-icon" /> Check Out The Wiki!</button
                >
            </div>
            <div class="card h-1/2 p-3 rounded-xl">
                ...
                {#each statistics as stat}
                    <div
                        class="flex flex-row align-middle items-center justify-between text-left mb-2.5"
                    >
                        <h2>{stat.name}</h2>
