...
<div
    class="flex grow justify-center flex-col gap-y-3.5 items-center overflow-y-auto"
>
    {#each Object.entries(settings) as [name, settingCategory]}
        <div class="card w-3/4 p-2">
            <h2 class="text-center text-3xl mb-2.5">{name}</h2>
            <div class="bg-white/25 mb-5 w-full h-px rounded-md"></div>
            {#each settingCategory as setting}
                <div class="flex justify-between items-center px-4 mb-3 mt-3">
                    <div class="w-full">
                        ...
                        <p class="text-sm text-gray-400 text-wrap max-w-2/3">
                            {setting.description}
                        </p>
                    </div>
                    ...
                    {:else if setting.type === "string"}
                        {#await setting.getValue() then value}
                            <input
                                type="text"
                                {value}
                                class="max-w-1/12 bg-tertiary dark:bg-tertiary-d py-1.5 px-2 rounded-sm"
                                onchange={async (e) =>
                    ...
                    {:else if setting.type === "button"}
                        <button class="app-btn" onclick={setting.onClick}
                            ><setting.icon class="button-icon" />
                        ...
                        <div class="dropdown">
                            {#await setting.getValue() then value}
                                <Dropdown
                                    ...
                                    startingIndex={setting.choiceValues.indexOf(
                                        value,
                                    )}
                                    onChange={async (index, _label) => {
                                        await setting.onChange(
                                            setting.choiceValues[index],
                                        );
                                    }}
                                />
                            {/await}
