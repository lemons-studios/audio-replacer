<script lang="ts">
    ...
    async function importData() {
        ...
        const overwrite = await ask(
            "Would you like to overwrite the filters in this project with the ones in this file?",
            {
                title: "Overwrite?",
                kind: "info",
            },
        ...
        for (let i = 0; i < obj.pitchFilters.length; i++) {
            ...
            if (
                ...
                (await validateFilter(o.pitchFilters[i].value))
            ) {
        ...
        for (let i = 0; i < obj.effectFilters.length; i++) {
            ...
            if (
                o.hasOwnProperty("name") &&
                o.hasOwnProperty("value") &&
                (await validateFilter(o.effectFilters[i].value))
            ) {
        ...
        if (!overwrite) {
            ...
            notificationManager.addToNotification(
                "success",
                "Success!",
                "Imported filters",
            );
            ...
            await setProperties(
                validProperties.pitchFilters,
                validProperties.effectFilters,
            );
            notificationManager.addToNotification(
                "success",
                "Success!",
                "Imported and overwritten filters",
            );
        }
    }
    ...
    async function removeEffect(index: number) {
        ...
        if (await confirm()) {
            ...
            const names =
                selectedTab === 1 ? effectFilterNames : pitchFilterNames;
            ...
            await updateArprojStats(
                selectedTab === 1 ? "effectFilters" : "pitchFilters",
                newEffects,
            );
            updateFilters();
        }
...
{#if projectLoaded}
    ...
    <div
        class="flex flex-row w-full justify-around px-4 py-2 gap-3 min-h-15 card mb-1.5"
    >
        <button
    ...
    <div class="card p-5 h-full">
        <div
            class="flex flex-col w-full rounded-lg justify-center items-center text-center gap-5"
        >
            {#each selectedTab === 0 ? pitchNames : effectNames as name, index}
                <div
                    class="flex flex-row items-center text-center justify-between min-h-17 dark:bg-tertiary-d bg-tertiary p-3 w-full rounded-lg"
                >
                    <h2>{name}</h2>
                    <div
                        class="gap-3 flex flex-row justify-center items-center"
                    >
                        {#if selectedTab === 0}
                            <h2>{pitchValues[index]}x</h2>
                        {:else}
                            <h2 class="text-center">
                                {`${effectValues[index] === "" ? "No Filter" : "Effect Value:"}`}
                        ...
                        <div class="flex flex-row w-auto">
                            <button
                                class="hover:bg-accent focus:bg-accent-secondary dark:focus:bg-accent-tertiary transition p-2 rounded-md"
                                onmouseleave={(e) => e.currentTarget.blur()}
                                onclick={async (e) => {
                                    e.currentTarget.blur();
                                    await removeEffect(index);
                                }}
                            >
            ...
            <div class="flex justify-end items-center w-full h-auto gap-2.5">
                ...
                <button
                    class="w-1/10 border-white/10 border transition duration-200 bg-accent hover:bg-accent-secondary dark:hover:bg-accent-tertiary dark:focus:bg-tertiary-d focus:bg-tertiary px-3 py-1.5 rounded-sm flex flex-row text-center items-center justify-center gap-2"
                    onclick={(e) => {
                        e.currentTarget.blur();
                        editEffect(null);
                    }}
