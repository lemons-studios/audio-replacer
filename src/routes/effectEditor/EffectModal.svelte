<script lang="ts">
    ...
    onMount(() => {
        if (selectedIndex !== null) {
            selectedValue.value = (isEffect ? effectFilters : pitchFilters)[
                selectedIndex
            ];
            selectedName.value = (
                isEffect ? effectFilterNames : pitchFilterNames
            )[selectedIndex];
        }
    });
    ...
    async function modifyEffect() {
        ...
        const effectToVerify = isEffect
            ? selectedValue.value
            : `rubberband=pitch=${selectedValue.value}`;
        ...
        await updateArprojStats(
            isEffect ? "effectFilters" : "pitchFilters",
            newList,
        );
        modal.toggleModal();
    }
...
<Modal closeable={false} bind:this={modal}>
    <div class="flex flex-col items-stretch justify-between w-full h-full">
        <h1 class="text-center text-4xl font-bold">
            {selectedIndex === null ? "Create" : "Edit"}
            {isEffect ? "Effect" : "Filter"}
        </h1>
        <!--Options-->
        <div
            class="flex flex-col justify-center text-left w-full h-full gap-y-5 mb-5"
        >
            <div class="w-full">
                <h1 class="text-xl font-bold mb-2">Name</h1>
                <input
                    bind:this={selectedName}
                    type="text"
                    maxlength="16"
                    placeholder="Epic Filter Name"
                    class="bg-tertiary w-full dark:bg-tertiary-d py-1.5 px-2 rounded-sm"
                />
            </div>
            <div>
