...
import {
  ...
  checkPermission,
  requestPermission,
} from "tauri-plugin-audio-recorder-api"; // This plugin is a godsend. Thank you to whoever made this
...
export async function startCapture() {
  ...
  if (!permission.granted) {
    ...
    if (!result.granted) {
      await message(
        "Audio Replacer requires access to the microphone to function. App will now close",
        { title: "Mic Required", kind: "error" },
      );
...
export async function endRecording(
  ...
  if (status.state === "recording") {
    ...
    const allowNoiseSuppression = await getValue(
      "settings.allowNoiseSuppression",
    );
    if (allowNoiseSuppression) {
      const noiseSuppressionFile = await resolveResource(
        "binaries/noiseSuppression.rnnn",
      );
      const intensity = await getValue("settings.noiseSuppressionIntensity");
      await applyFFMpegFilter(
        `arnndn=model=${noiseSuppressionFile}:mix=${intensity}`,
      );
    }

    // Pitch Shift
    await applyFFMpegFilter(
      `rubberband=pitch=${pitchFilters[selectedPitchIndex]}`,
    );
