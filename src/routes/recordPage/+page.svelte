<script lang="ts">
        ...
        recording: [
            ...
            },
            {
                ...
                action: async () => {
                    ...
                    const autoAcceptRecordings = await getValue(
                        "settings.autoAcceptRecordings",
                    );
        ...
        reviewing: [
            ...
            },
            {
                ...
                action: async () => {
                    ...
                    audioSource =
                        audioSource === outputFile ? currentFile : outputFile;
                    notificationManager.addToNotification(
                        "info",
                        "Switched",
                        `You are now viewing ${audioSource === outputFile ? "your recording" : "the original file"}`,
                        true,
                        5000,
                    );
                },
    ...
    function updateContent() {
        ...
        audioSource = currentFile;
        setPresenceState(
            `Files Remaining: ${filesRemaining} (${progressPercentage})`,
        );
    }
    ...
    onMount(async () => {
        ...
        await setPresenceDetails(
            `Recording Audio in ${await getArprojProperty("name")}`,
        );
...
{#if projectLoaded}
    ...
    <div class="flex flex-row gap-5 h-full">
        <div
            class="flex flex-col justify-center items-center w-5/8 card rounded-lg"
        >
            <h1 class="font-medium text-2xl text-center">{file}</h1>
            <h3 class="font-light text-sm text-gray-400 mb-5">
                Files Remaining: {filesRemaining} ({progressPercentage})
            </h3>
            <div
                class="flex flex-row gap-1.5 w-8/10 justify-center mb-15 text-center items-center dark:text-gray-500 text-gray-700 text-xs"
            >
                0% <ProgressBar completion={rawProgress} /> 100%
            </div>
            <AudioPlayer source={audioSource} />
            <h3
                class=" text-gray-800 dark:text-gray-300 text-center w-4/5 mb-5"
            >
                {transcription}
            </h3>
        ...
        <div
            class="flex flex-col justify-center items-center w-3/8 card rounded-lg gap-y-10"
        >
            <div>
            ...
            <div class="flex flex-row gap-x-3.5">
                <ToggleSwitch
                    onClick={(extraEdits = !extraEdits)}
                    enabled={extraEdits}
                />
                <p>Extra Edits Required?</p>
            </div>
            <button
                class="app-btn-secondary min-w-60"
                onmouseleave={(e) => e.currentTarget.blur}
                onclick={async (e) => {
                    e.currentTarget.blur();
